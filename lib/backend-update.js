...
function checkLatestInCloud(callback){
    backendRetrieve.getLatestBackendDetails(_projectInfo, function(backendDetails){
        _backendProjectDetails = backendDetails
        if(projectInfoManager.checkBackendProjectLastUpdateTime(_projectInfo, backendDetails)){
            //nothing is changed, but just to make sure the user didn't delete or modify this file
            //because it is used by the following push procedure
            backendInfoManager.setBackendDetails(_projectInfo.ProjectPath, backendDetails)
            callback()
        }else{
