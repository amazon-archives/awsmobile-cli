...
function syncCurrentBackendInfo(projectInfo, backendDetails, awsConfig, syncToDevFlag, callback) {
    if(backendDetails && backendDetails.projectId && backendDetails.projectId.length > 0){
        const currentBackendInfoDirPath = pathManager.getCurrentBackendInfoDirPath(projectInfo.ProjectPath)
...
function onComplete(){
    ...
    inquirer.prompt([
    {
        ...
        if(answers.syncBackendSpecWithInfo){
            let cloudProject = awsMobileYamlOps.readYamlFileSync(pathManager.getCurrentBackendYamlFilePath(_projectInfo.ProjectPath))
