...
function isNewBuildNeeded(lambdaFuncName){
    ...
    if(srcCodeDir && fs.existsSync(srcCodeDir)){
        if(isCodebaseChanged(lambdaFuncName){
            result = isZipFileOutOfDate(lambdaFuncName)
        }
    }
...
function isZipFileOutOfDate(lambdaFuncName){
    let result = false
    ...
    if(existingZipFilePath && fs.existsSync(existingZipFilePath)){ 
        ...
        result =!lastBackendBuildTime.isValid() ||
                ...
                lastBackendBuildTime.isBefore(zipFileModificationTime)
...
function getTimeStamp(projectInfo){
    ...
    if(lastBackendPushTime.isValid()){
        if(result.isValid()){
    ...
    if(lastBackendSyncToDevTime.isValid()){
        if(result.isValid()){
