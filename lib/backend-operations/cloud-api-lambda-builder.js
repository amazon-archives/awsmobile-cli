...
function isNewBuildNeeded(lambdaFuncName){
    ...
    if(srcCodeDir && fs.existsSync(srcCodeDir)){
        if(existingZipFilePath && fs.existsSync(existingZipFilePath)){ 
            //initialization can change backend project
            let projectInitTime = moment(_projectInfo.InitializationTime, awsmobileJSConstant.DateTimeFormatString) 
            let lastBackendUpdateTime = moment(_projectInfo.BackendLastPushTime, awsmobileJSConstant.DateTimeFormatString)
            ...
            result =!projectInitTime.isValid() ||
                    !lastBackendUpdateTime.isValid() ||
                    !lastSrcDirModificationTime.isValid() ||
                    !zipFileModificationTime.isValid() ||
                    lastBackendUpdateTime.isBefore(projectInitTime) ||
                    lastBackendUpdateTime.isBefore(lastSrcDirModificationTime) ||
                    lastBackendUpdateTime.isBefore(zipFileModificationTime) //user manually changed build
        }else{
            result = true
        }
    }
