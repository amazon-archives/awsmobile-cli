...
function createDDBTables(appsyncCreationHandle){
  ...
  appsyncCreationHandle.settings.DataSources.forEach(dataSource => {
    if(dataSource.type == 'AMAZON_DYNAMODB'){
      dataSource.spec = appsyncManager.getDynamoDBSpec(appsyncCreationHandle.projectInfo.ProjectPath, dataSource.name)
      dataSource.spec.TableName += tableNameSuffix
...
function createGraphqlApi(appsyncCreationHandle){
  ...
  return new Promise((resolve, reject) => {
    if(appsyncCreationHandle.settings.APIName == "{auto-generate}"){
      appsyncCreationHandle.settings.APIName = nameManager.generateGraphqlAPIName(appsyncCreationHandle.projectInfo)
    }
    let param = {
...
function onSuccess(appsyncCreationHandle){
  ...
  console.log('appsync creation complete: ' + chalk.blue(appsyncCreationHandle.settings.APIName))
}
