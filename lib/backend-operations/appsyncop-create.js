...
const helper = require('./appsync-service-role-helper.js')
...
function createDDBTable(appsyncCreationHandle, spec){
  return new Promise((resolve, reject) => {
    ...
    dynamoDBClient.createTable(param, (err, data)=>{
      if(err){
        console.log(chalk.red('Failed to create dynamoDB table ' + spec.TableName))
        console.log(err)
        reject(err)
      }else{
        ...
        appsyncCreationHandle.createDDBTableResponse[spec.TableName] = data
        console.log('create table ' + spec.TableName + ' response')
        console.log(data)
...
function createServiceRole(appsyncCreationHandle){
  console.log('create service role ......')
  return new Promise((resolve, reject) => {
    let param = helper.constructCreateRoleParam(appsyncCreationHandle)
    ...
    iamClient.createRole(param, (err, data)=>{
      if(err){
        ...
        appsyncCreationHandle.createServiceRoleResponse = data
        resolve(appsyncCreationHandle)
      }
    })
  })
