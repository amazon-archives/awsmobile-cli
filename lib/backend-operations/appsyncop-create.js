...
function putRolePolicies(appsyncCreationHandle){
  ...
  let paramList = serviceRoleHelper.constructPutRolePolicyParamList(appsyncCreationHandle)
  let putRolePolicyTasks = []
  paramList.forEach(param => {
    putRolePolicyTasks.push(putRolePolicy(appsyncCreationHandle, param))
  })
...
function putRolePolicy(appsyncCreationHandle, param){
  console.log('put service role policy ' + param.RoleName)
  console.log(param)
  return new Promise((resolve, reject) => {
    resolve(appsyncCreationHandle)
    let iamClient = awsClient.IAM(appsyncCreationHandle.awsDetails)
    iamClient.putRolePolicy(param, (err, data)=>{
      if(err){
        console.log(chalk.red('put service role policy ' + param.RoleName))
        console.log(err)
        reject(err)
      }else{
        if(!appsyncCreationHandle.putRolePolicyResponse){
          appsyncCreationHandle.putRolePolicyResponse = {}
        }
        appsyncCreationHandle.putRolePolicyResponse[param.RoleName] = data
        console.log('put role policy ' + param.RoleName + ' response')
        console.log(data)
        resolve(appsyncCreationHandle)
      }
    })
