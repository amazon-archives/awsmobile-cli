...
function onSuccess(appsyncRetrieveHandle){
  ...
  appsyncRetrieveHandle.listResolversResponse.forEach(resolver=>{
    writeResolverMappings(appsyncRetrieveHandle, currentInfoDirPath, resolver)
  })
...
function writeResolverMappings(appsyncRetrieveHandle, currentInfoDirPath, resolver){
  let resolverMappingsDirPath = path.join(currentInfoDirPath, awsmobileJSConstant.resolverMappingsDirName)
