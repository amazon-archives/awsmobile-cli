...
function run(projectInfo, awsDetails){
  ...
  return updateDDBTables(appsyncUpdateHandle)
...
function updateDDBTables(appsyncCreationHandle){
  ...
  appsyncCreationHandle.dataSources.forEach(dataSource => {
    if(dataSource.type == 'AMAZON_DYNAMODB'){
      ...
      createTableTasks.push(updateDDBTable(appsyncCreationHandle, dataSource))
    }
  })
...
function updateDDBTable(appsyncCreationHandle, dataSource){
...
function updateLambdaFunction(appsyncCreationHandle){
  return new Promise((resolve, reject) => {
    resolve(appsyncCreationHandle)
  })
}
...
function updateGraphqlApi(appsyncCreationHandle){
...
function updateDataSources(appsyncCreationHandle){
  console.log('appsync creation: create data sources...')
  let createDataSourceTasks = []
  appsyncCreationHandle.dataSources.forEach(dataSource => {
    if(dataSource.type == 'AMAZON_DYNAMODB'){
...
function updateDataSource(appsyncCreationHandle, dataSource){
  return new Promise((resolve, reject) => {
    let param = {
      apiId: appsyncCreationHandle.createGraphqlApiResponse.graphqlApi.apiId,
      name: dataSource.name,
...
function updateResolvers(appsyncCreationHandle){
  console.log('appsync creation: create resolvers...')
  let createResolverTasks = []
  appsyncCreationHandle.resolvers.forEach(resolver => {
    createResolverTasks.push(createResolver(appsyncCreationHandle, resolver))
...
function updateResolver(appsyncCreationHandle, resolver){
  return new Promise((resolve, reject) => {
    let param = {
      apiId: appsyncCreationHandle.createGraphqlApiResponse.graphqlApi.apiId,
      typeName: resolver.typeName,
...
function updateApiKey(appsyncCreationHandle){
