...
function diff(appsyncUpdateHandle){
    ...
    appsyncUpdateHandle.devAppsyncInfo.apiKeys = diffMarkedApiKeys
}
...
function markDiff(devApiKey, currentApiKey){
    ...
    if(!currentApiKey){
        ...
        if(devApiKey.description == currentApiKey.description && devApiKey.expires == currentApiKey.expires){
            devApiKey[DIFF] = NONE
