...
function diff(appsyncUpdateHandle){
  ...
  return new Promise((resolve, reject) => {
    helperApiKeys.diff(appsyncUpdateHandle)
    helperDataSources.diff(appsyncUpdateHandle)
    helperDynamoDB.diff(appsyncUpdateHandle)
    helperGraphqlApi.diff(appsyncUpdateHandle)
    helperResolvers.diff(appsyncUpdateHandle)
    helperSchema.diff(appsyncUpdateHandle)
    resolve(appsyncUpdateHandle)
  })
}
...
function onSuccess(appsyncUpdateHandle){
  ...
  console.log(util.inspect(appsyncUpdateHandle, false, null))
}
