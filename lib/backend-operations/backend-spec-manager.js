...
function synSpecWithCurrentBackendSpec(projectInfo, callback){
    let backendProject = awsMobileYamlOps.readYamlFileSync(pathManager.getCurrentBackendYamlFilePath(projectInfo.ProjectPath))
    if(backendProject){
        setBackendProjectObject(backendProject, projectInfo)
    }
    if(callback){
...
function getBackendProjectObject(projectInfo)
{
    ...
    if(fs.existsSync(backendProjectYamlFilePath)){
        ...
        backendProject = awsMobileYamlOps.readJsonFileSync(backendProjectJsonFilePath)
    }else{
...
function onBackendFormatChange(projectInfo)
{
    ...
    if(fs.existsSync(backendProjectYamlFilePath) && projectInfo.BackendFormat == backendFormats.Json){
        backendProject = awsMobileYamlOps.readYamlFileSync(backendProjectYamlFilePath)
        if(backendProject){
            awsMobileYamlOps.writeJsonFileSync(backendProject, backendProjectJsonFilePath)
