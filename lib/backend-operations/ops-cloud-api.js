...
function build(projectInfo, backendProject, callback){
    ...
    if(isNewBuildNeeded()){
        ...
        lambdaBuilder.build(projectInfo, backendProject, _featureName, callback)
    }else{
        if(callback){
            callback()
        }
...
function preBackendUpdate(projectInfo, awsConfig, backendProjectDetails, callback) {
    lambdaUploader.uploadLambdaZipFiles(projectInfo, awsConfig, backendProjectDetails, _featureName, callback)
}

//////////////////// sync backend project ////////////////////
function syncCurrentBackendInfo(projectInfo, backendDetails, awsConfig, callback){
    ...
    if(lambdaFunctions && lambdaFunctions.length > 0){
        ...
        lambdaFunctions.forEach(function(lambdaFunction){
            downloadLambdaFunction(projectInfo, lambdaFunction, function(){
                count ++
...
function downloadLambdaFunction(projectInfo, lambdaFunction, callback){
    const getParams = { FunctionName: lambdaFunction.name }
    const lambdaClient = awsClient.Lambda(projectInfo, lambdaFunction.attributes.region)
    ...
    if(fs.existsSync(_featureDirPath)){
        if(fs.existsSync(_featureBuildDirPath)){
            ...
            result =!lastBackendBuildTime.isValid() ||
                    ...
                    lastBackendBuildTime.isBefore(lastFeatureModificationTime) ||
                    lastBackendBuildTime.isBefore(lastFeatureBuildModificationTime) //user manually changed build
        }else{
            result = true
        }
    }else{ // the user has removed the feature dir
        fs.mkdirsSync(_featureDirPath)
        result = false
    }
