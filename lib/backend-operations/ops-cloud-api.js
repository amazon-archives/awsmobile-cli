...
function isNewBuildNeeded(){
    ...
    if(fs.existsSync(_featureDirPath)){
        if(fs.existsSync(_featureBuildDirPath)){    
            let lastBackendPushTime = moment(_projectInfo.BackendLastPushTime, awsmobileJSConstant.DateTimeFormatString)
            let lastBackendSyncTime = moment(_projectInfo.BackendLastSyncTime, awsmobileJSConstant.DateTimeFormatString)
            if(lastBackendPushTime.isValid() && lastBackendSyncTime.isValid()){
                let timeStamp = lastBackendPushTime > lastBackendSyncTime ? lastBackendPushTime : lastBackendSyncTime
                ...
                result =!lastBackendSpecModificationTime.isValid() ||
                        !lastFeatureModificationTime.isValid() ||
                        !lastFeatureBuildModificationTime.isValid() ||
                        timeStamp.isBefore(lastBackendSpecModificationTime) ||
                        timeStamp.isBefore(lastFeatureModificationTime) ||
                        timeStamp.isBefore(lastFeatureBuildModificationTime) //user manually changed build
            }else{
                result = true
            }
        }else{
