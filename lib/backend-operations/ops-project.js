...
function build(projectInfo, backendProject, ensureBuildZipFileFlag, callback){
    if(isNewBuildNeeded(projectInfo, ensureBuildZipFileFlag)){
...
function isNewBuildNeeded(projectInfo, ensureBuildZipFileFlag){
    let result = false
    
    let timeStamp = getTimeStamp(projectInfo)
    ...
    if(fs.existsSync(backendContentZipFilePath)){
        ...
        result =!timeStamp.isValid() || 
                ...
                timeStamp.isBefore(lastBackendBuildZipFileMTime) //user manually changed build
    }else if(!ensureBuildZipFileFlag){
        result =!timeStamp.isValid() || 
                !lastBackendSpecModificationTime.isValid() ||
                timeStamp.isBefore(lastBackendSpecModificationTime)
    }else{
