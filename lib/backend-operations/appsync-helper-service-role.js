...
function constructCreateRoleParamList(appsyncCreationHandle)
{
    ...
    Object.keys(appsyncCreationHandle.createDDBTableResponse).forEach(TableName => {
        paramList.push(constructCreateRoleParamForDDB(appsyncCreationHandle.createDDBTableResponse[TableName].TableDescription))
    })
    return paramList
}
...
function constructCreateRoleParamForDDB(tableDescription){
    let result = {
        RoleName: constructDDBRoleName(tableDescription), 
...
function constructDDBRoleName(tableDescription){
    let dataSource = {
