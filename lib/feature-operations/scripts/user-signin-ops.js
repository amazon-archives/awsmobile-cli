...
var saveSettings = (currentDefinition) => {
  ...
  return Promise.resolve(currentDefinition['sign-in']);
}
...
var startDefaultSettings = (currentDefinition) => new Promise(
  function(resolve, reject) {
    currentDefinition['sign-in']['components']['sign-in-user-pools'] =
      new mhYamlLib.UserPoolsIdentityProvider({
        'alias-attributes': [ 'email', 'phone_number' ],
        'mfa-configuration': 'ON',
        'name': 'userpool',
        'password-policy': new mhYamlLib.ConvertibleMap({
          'min-length': 8,
...
var usersLoginQuestionary = (currentDefinition) => {
  ...
  return inquirer.prompt(userpoolsQuestion).then(answers => {
    if (answers.selection.length === 0) {
      ...
      currentDefinition['sign-in']['components']['sign-in-user-pools']['attributes']['alias-attributes'] = answers.selection;
...
var mfaConfiguration = (currentDefinition) => {
  ...
  if (!phoneRequired) {
    ...
    return inquirer.prompt(mfaQuestion).then(answer => {
      currentDefinition['sign-in']['components']['sign-in-user-pools']['attributes']['mfa-configuration'] = answer.options;
      return Promise.resolve(currentDefinition);
    });
  }
};
...
var passwordPolicy = (currentDefinition) => {
  ...
  return inquirer.prompt(passwordQuestions).then(answers => {
    ...
    answers.selection.forEach(requirement => {
...
var googleSettings = (currentDefinition) => {
  ...
  return inquirer.prompt(googleQuestion).then(answer => {
    currentDefinition['sign-in']['components']['sign-in-google'] =
      new mhYamlLib.OpenIDConnectIdentityProvider({
        'open-id-connect-audience-webapp': answer.webapp,
        'open-id-connect-audience-ios': answer.ios,
        'open-id-connect-audience-android': answer.android,
...
var facebookSettings = (currentDefinition) => {
  ...
  return inquirer.prompt(facebookQuestion).then(answer => {
    currentDefinition['sign-in']['components']['sign-in-facebook'] =
      new mhYamlLib.StandardIdentityProvider({
        'provider-id': answer.app_id,
...
function main(projectInfo) {
  var currentDefinition = {
    'sign-in': new mhYamlLib.SignIn({
      attributes: {
        enabled: false,
  ...
  mhYamlLib.load(_projectInfo, (yamlDefinition) => {
    ...
    //console.log(JSON.stringify(currentDefinition, null, 2));
