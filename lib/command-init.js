...
function init(mobileProjectID){
    ...
    if(projectValidator.validate(projectPath)){
        ...
        mobile.listProjects( {}, (err, list) => {
            if (!list && list.projects && list.projects.size() === 0) {
                runInit(projectPath, mobileProjectID)
            } else {
                let confirmMessage = 'Do you want to link to an existing Mobile Hub project?'
                inquirer.prompt([
                    {
                        type: 'confirm',
                        name: 'linkExistingProject',
                        message: confirmMessage,
                        default: true
                    }
                ]).then(function (answers) {
                    if(answers.linkExistingProject){
                        let projects = [];
                        let maxNameSize = 0;
                        list.projects.forEach(element => {
                            if (maxNameSize < element.name.length) {
                                maxNameSize = element.name.length;
                            }
                        });
                        const spaces = "                                                                                                    ";
                        list.projects.forEach(element => {
                            element.value = element.projectId;
                            element.name = element.name + spaces.substring(0, maxNameSize - element.name.length) + " (" + element.projectId + ")";
                            projects.push(element);
                        });
                        let selectProjectMessage = 'Select the project you want to link'
                        inquirer.prompt([
                            {
                                type: 'list',
                                name: 'selectProject',
                                message: selectProjectMessage,
                                choices: projects
                            }
                        ]).then(function (answers) {
                            runInit(projectPath, answers.selectProject);
                        })
                    } else {
                        console.log()
                        runInit(projectPath, mobileProjectID)
...
function setupAmplifyDependency(projectPath){
    ...
    if(fs.existsSync(packageJsonFilePath)){
        ...
        packageObj.dependencies["aws-amplify"] = "^0.1.22"
