...
function resolve_project_awsConfig(projectInfo){
    ...
    if(!fs.existsSync(awsInfoFilePath)){
        ...
        awsInfo.AWSInfoFilePath = awsInfoFilePath
        awsInfo.AWSConfigFilePath = path.normalize(path.join(pathManager.getSysAWSMobileJSDirPath(), nameManager.generateAWSConfigFileName(projectInfo)))
...
function isSyncProfileNeeded(awsInfo){
    ...
    console.log('isSyncNeeded', result)
    ...
    if(fs.existsSync(sysConfigFilePath)){
        try{
            ...
            if(!result){
                ...
                console.log('aws-cli config file querried:')
