...
function setProfile(profileName){
    ...
    if(executeSet){
        ...
        if(!awsInfo.AWSConfigFilePath){
            awsInfo.AWSConfigFilePath = path.normalize(path.join(pathManager.getProjectAWSConfigDirPath(), nameManager.generateAWSConfigFileName(projectInfo)))
        }
...
function resolve_project_awsConfig(projectInfo){
    ...
    if(!fs.existsSync(awsInfoFilePath)){
        ...
        awsInfo.AWSInfoFilePath = awsInfoFilePath
        awsInfo.AWSConfigFilePath = path.normalize(path.join(pathManager.getProjectAWSConfigDirPath(), nameManager.generateAWSConfigFileName(projectInfo)))
        console.log(awsInfo.AWSConfigFilePath)
