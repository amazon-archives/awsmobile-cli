...
function Mobile(projectInfo){
    const awsConfigFilePath = awsConfigManager.getAWSConfigFilePath(projectInfo)
...
function S3(projectInfo, region){
    const awsConfigFilePath = awsConfigManager.getAWSConfigFilePath(projectInfo)
    AWS.config.loadFromPath(awsConfigFilePath)
    AWS.config.update({customUserAgent: awsmobileJSConstant.CustomUserAgent})
    if(region){
...
function Lambda(projectInfo, region){
    const awsConfigFilePath = awsConfigManager.getAWSConfigFilePath(projectInfo)
    AWS.config.loadFromPath(awsConfigFilePath)
    if(region){
        AWS.config.update({region: region})
...
function CloudFront(projectInfo, region){
    const awsConfigFilePath = awsConfigManager.getAWSConfigFilePath(projectInfo)
    AWS.config.loadFromPath(awsConfigFilePath)
    AWS.config.update({customUserAgent: awsmobileJSConstant.CustomUserAgent})
    if(region){
