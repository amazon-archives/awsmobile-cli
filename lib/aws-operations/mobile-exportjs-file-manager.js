...
function getAWSExportFile(projectInfo, callback){
    _projectInfo = projectInfo
    if(projectInfo.BackendProjectID && projectInfo.BackendProjectID.length > 0){
        retrieveAWSExportFile(callback)
    }
}
...
function retrieveAWSExportFile(callback)
{
    let spinner = ora('retrieving aws-exports.js')
    spinner.start()

    let mobile = awsClient.Mobile(_projectInfo)
            ...
            extract(tempZipFilePath, {dir: tempExtractDirPath}, function (err) {
                if(err){
                    ...
                    if(tempExportFilePath){
                        updateLocalAWSExportFiles(tempExportFilePath)
...
function updateLocalAWSExportFiles(sourceFilePath){ 
    ...
    if(srcDirExportFilePath && fs.existsSync(_projectInfo.SourceDir)){
        fs.copySync(sourceFilePath, srcDirExportFilePath)
        console.log('awsmobile project\'s access information copied to: ' + 
        chalk.blue(pathManager.getSrcDirExportFilePath_relative(_projectInfo)))    
    }
}
