...
function setProfile(profileName){
    ...
    let awsConfig = awsConfigProfileReader.getSystemConfig(profileName, silentFlag)
    if(awsConfig){
        ensureFolderStructure()
        ...
        if(executeSet){
            ...
            fs.writeFileSync(awsConfigFilePath, awsConfigJsonString, 'utf8')
        }
    }else{
        isSuccessful = false
    }
...
function syncProfile(awsInfo, silentFlag){
    let awsConfig = awsConfigProfileReader.getSystemConfig(awsInfo.ProfiletName, silentFlag)
    if(awsConfig){
        let jsonString = JSON.stringify(awsConfig)
