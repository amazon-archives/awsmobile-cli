...
function checkAWSConfig(callback){
    ...
    if(awsConfigFileManager.validateAWSConfig(awsConfig)){
        callback(awsConfig)
    }else{
...
function configureAWS(profileName, callback){
    if(profileName){
        ...
        let isNewUser = false
        if(!awsConfigFileManager.validateAWSConfig(awsConfig)){ //assume new user
            isNewUser = true
        ...
        inquirer.prompt([
            ...
            if(awsConfigChanged){
                ...
                awsConfigFileManager.setNoProfile(awsInfo)
            }
