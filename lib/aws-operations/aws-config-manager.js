...
function checkAWSConfig(callback){
    ...
    if(awsConfigFileManager.validateAWSConfig(awsDetails)){
        callback(awsDetails)
    }else{
...
function newUserSetup(){
    ...
    if(awsConfigFileManager.validateAWSConfig(awsDetails)){
        ...
        console.log()
        console.log(chalk.gray('# to use the credentials of named profile'))
...
function newUserSteps(awsDetails){
    ...
    console.log('Please first sign up/in your aws account with Administrator Access:')
    console.log(chalk.green(awsmobileJSConstant.AWSAmazonUrl))
    opn(awsmobileJSConstant.AWSAmazonUrl, {wait: false})
    PressAnyKeyToContinue('Press any key to continue after signin', function(){
        console.log('It is adviced that you create a new IAM user for awsmobile access')
        inquirer.prompt([
            {
                type: 'input',
                name: 'userName',
            ...
            console.log('Please enter the access key info you find on the console:')
