...
function run(projectPath, mobileProjectID){
    return new Promise((resolve, reject)=>{
        if(fs.existsSync(projectPath)){
            let initInfo = {
                ...
                backendProject: getBackendProject(projectPath),
                packageJson: getPackageJson(projectPath)
            }
...
function getProjectInfo(projectPath){
    let result 
    let projectInfo = readJsonFile(pathManager.getProjectInfoFilePath(projectPath))
...
function getProjectConfig(projectPath){
    let result 
    let config = readJsonFile(pathManager.getProjectConfigFilePath(projectPath))
...
function getPackageJson(projectPath){
    return readJsonFile(path.normalize(path.join(projectPath, 'package.json')))
}
