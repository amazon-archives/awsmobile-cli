...
function updateAppSyncDetails(projectInfo, appsyncDetails){
    Object.keys(appsyncPropertyMappings).forEach(function(propertyName) {
        projectInfo[propertyName] =  appsyncDetails[appsyncPropertyMappings[propertyName]]
    })
    projectInfo.AppsyncConsoleUrl = awsmobileJSConstant.AppsyncConsoleUrl.replace('{apiId}', appsyncDetails.apiId).replace('{region}', appsyncDetails.region)
    setProjectInfo(projectInfo)
    return projectInfo
}


function clearAppSyncDetails(projectInfo){
    Object.keys(appsyncPropertyMappings).forEach(function(propertyName) {
        projectInfo[propertyName] =  ""
    })
    setProjectInfo(projectInfo)
