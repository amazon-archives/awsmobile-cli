...
function getAppSyncInfo(projectInfo){
    let appsyncInfo = {}

    Object.keys(appsyncPropertyMappings).forEach(function(propertyName) {
        appsyncInfo[appsyncPropertyMappings[propertyName]] = projectInfo[propertyName]
    })

    return appsyncInfo
}

function updateAppSyncInfo(projectInfo, appsyncInfo){
    console.log('update appsync info')
    console.log(appsyncInfo)
    Object.keys(appsyncPropertyMappings).forEach(function(propertyName) {
        projectInfo[propertyName] =  appsyncInfo[appsyncPropertyMappings[propertyName]]
    })
    projectInfo.AppsyncConsoleUrl = awsmobilejsConstant.AppsyncConsoleUrl.replace('{apiId}', appsyncInfo.apiId).replace('{region}', appsyncInfo.region)
    setProjectInfo(projectInfo)
    return projectInfo
}


function clearAppSyncInfo(projectInfo){
    Object.keys(appsyncPropertyMappings).forEach(function(propertyName) {
        projectInfo[propertyName] =  ""
    })
    setProjectInfo(projectInfo)
...
const appsyncPropertyMappings = {
    "AppsyncApiId": "apiId",
    "AppsyncApiName": "name",
    "AppsyncRegion": "region",
    "AppsyncGraphqlEndpoint": "graphqlEndpoint",
    "AppsyncAuthenticationType": "authenticationType",
    "AppsyncApiKey":  "apiKey",
    "AppsyncCreationTime": "creationTime",
    "AppsyncLastUpdatedTime": "lastUpdateTime",
    "AppsyncLastSyncTime": "lastSyncTime",
    "AppsyncLastSyncToDevTime": "lastSyncToDevTime",
}
