...
let _yesFlag
let _repo
function start(projectName, starterName, yesFlag){
    if(!projectName){
...
function setupBackend(projectInfo, contentZipFilePath, callback){
    backendCreate.createBackendProject(projectInfo, {contentZipFilePath: contentZipFilePath, yesFlag: _yesFlag, syncToDevFlag: 1}, function(){
...
function printWelcomeMessage(projectInfo){
    ...
    console.log()
    console.log('   ' + chalk.blue('cd ' + chalk.blue(projectInfo.ProjectName)))
    console.log('   Run/Serve/Build your app')
    console.log()
