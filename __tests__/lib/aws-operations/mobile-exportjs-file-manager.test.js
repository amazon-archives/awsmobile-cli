...
const exportjsFileManager = require('../../../lib/aws-operations/mobile-exportjs-file-manager.js')
...
describe('mobile-exportjs-file-manager', () => {
    ...
    const mock_exportBundleResponse = {
    ...
    beforeAll(() => {
        ...
        const mock_mobileClient = {
            exportBundle: jest.fn((param, callback)=>{
                callback(null, mock_exportBundleResponse)
            })
        }
    ...
    test('getAWSExportFile', () => {
        let callback = jest.fn()
        exportjsFileManager.getAWSExportFile(mock_projectInfo, mock_awsDetails, callback)
        expect(callback).toBeCalled()
    })

    test('onClearBackend', () => {
        exportjsFileManager.onClearBackend(mock_projectInfo)
        expect(fs.removeSync).toBeCalled()
    })

    test('onProjectConfigChange', () => {
        exportjsFileManager.onProjectConfigChange(mock_projectInfo_old, mock_projectInfo)
        expect(fs.removeSync).toBeCalled()
